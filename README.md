...
You are now responsible for renaming structs in this whole application. The current naming schema in the codebase is inconsistent and causes unecessary frustration when trying to match it to a codegen solution. File called `generated.rs` is a generated version of v30 Bitcoin CLI, with a programmatic and coherent naming schema, which the rest of this application follow in some struct and doesnt not follow in many others. for examples structs that are items in list get prefixed with Item in codegen with 100% consistency, but the repo code here doesn't do it which makes it hard to verify if the implementation matches bitcoin core. So your task is to rename the bitcoin cli structs through the whole app (not the structs in generated.ts of course) so that it matches generated.rs.     

If you wish to modify, fix, evolve or improve problems in naming schema of generated.rs you can modify code in codegen/ folder. To generate again, run `./cmd.sh`. 
For you to check you did the right job you may do `cd integration_test; cargo test --features 30_0` to ensure that the app compiles after a set of name changes you did. feel free to run any command to check the code still compiles, but the litmus test is running successfully integration test at the end.
The end goal is to reduce to 0 the mismatches in compare.txt between generated code and repo implementation (which is also generated by ./cmd.sh)
Attention to detail is what I need from you.
WRITE A FILE EXPLAINING ALL THE CHANGES YOU DID IN `NAMING_FIX.md` workspace root. For naming changes of original repo structs you only need to write `<old_name>: <new_name>`. Any problems you could not fix on your own should be noted down on this file, wit ha full diagnosis of what the problem is. 


Could you implement a "bridge" that maps types of generated.rs to repo names? you have total freedom and its up to you to reduce the number of mismatches in "Structs only in SPEC" to ZERO! It really is up to you how to intelligently implement this.
